...
class PhotoAnalysisService {
  ...
  async comparePhotos(
    ...
    try {
      ...
      // Create comparison prompt for side-by-side analysis
      const comparisonPrompt = this.createComparisonPrompt(userHabits, userData);

      // Analyze both photos together using comparison prompt
      const comparisonResult = await this.callOpenAIVisionComparison(
        currentProcessed, 
        previousProcessed, 
        comparisonPrompt
      );
      ...
      return {
        ...
        cost: 0.02 // estimated cost for single comparison analysis
      };
...
`;
  ...
  private async callOpenAIVisionComparison(
    ...
    try {
      console.log('Calling OpenAI Vision API for comparison with image URL lengths:', 
        currentImageUrl.length, previousImageUrl.length);
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          console.log(`Attempt ${attempt}/${maxRetries} to call OpenAI Vision API for comparison`);
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a wellness app assistant that provides comparison analysis between two photos for skincare progress tracking. This is for personal wellness tracking only, not medical analysis. Focus on positive, encouraging feedback about visible changes between photos. Avoid medical terminology and provide supportive wellness suggestions. IMPORTANT: Always respond with ONLY valid JSON - no explanations, no markdown formatting, no additional text.'
          },
          {
            role: 'user',
            content: [
        ...
        temperature: 0.3,
          ...
          console.log('OpenAI Comparison API response status:', response.status);
          console.log('OpenAI Comparison API response headers:', Object.fromEntries(response.headers.entries()));

          if (!response.ok) {
            const errorData = await response.json();
            console.error('OpenAI Comparison API error response:', errorData);
            
            // Check for rate limiting
            if (response.status === 429 || errorData.error?.message?.includes('rate limit')) {
              console.warn('Rate limit hit during comparison, will retry after delay');
              if (attempt < maxRetries) {
                await new Promise(resolve => setTimeout(resolve, 2000 * attempt)); // Exponential backoff
                continue;
              }
            ...
            if (errorData.error?.message?.includes('policy') || 
                errorData.error?.message?.includes('violation') ||
                errorData.error?.message?.includes('refuse')) {
              console.error('Policy violation detected during comparison. This may be due to:');
              console.error('1. Medical terminology in prompts');
              console.error('2. Image content issues');
              console.error('3. API key restrictions');
              console.error('4. Rate limiting');
              ...
              throw new PhotoAnalysisError(
                `OpenAI policy violation during comparison: ${errorData.error?.message || 'Content policy violation'}`,
                'API_ERROR',
                errorData
              );
            }
            ...
            if (errorData.error?.message?.includes('quota') || errorData.error?.message?.includes('billing')) {
              throw new PhotoAnalysisError(
                `OpenAI quota exceeded during comparison: ${errorData.error?.message || 'Billing quota exceeded'}`,
                'INSUFFICIENT_CREDITS',
                errorData
              );
            }
            ...
            throw new PhotoAnalysisError(
              `OpenAI API error during comparison: ${errorData.error?.message || 'Unknown error'}`,
              'API_ERROR',
              errorData
            );
          }
          ...
          const data = await response.json();
          console.log('OpenAI Comparison API success response:', JSON.stringify(data, null, 2));
          
          const content = data.choices[0]?.message?.content;
          console.log('AI Comparison Response content:', content);
          ...
          if (attempt < maxRetries) {
            const delay = 1000 * Math.pow(2, attempt - 1); // 1s, 2s, 4s
            console.log(`Retrying comparison in ${delay}ms...`);
            await new Promise(resolve => setTimeout(resolve, delay));
          }
        }
      }
      ...
      // If all retries failed
      throw lastError || new Error('All retry attempts failed for comparison');
      
    } catch (error) {
      console.error('Error in callOpenAIVisionComparison:', error);
      throw error;
    }
  }
